...
// Sanity kurulana kadar geçici veriler
const modeller = [
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-800">
      ...
      <main className="flex-grow">
        {/* SLIDER BÖLÜMÜ (Boyutu Küçültüldü, Yazı Alta Alındı) */}
        <section className="max-w-5xl mx-auto px-4 sm:px-6 pt-6 pb-4">
          <div className="relative overflow-hidden rounded-xl bg-slate-900 shadow-sm h-[200px] sm:h-[300px] group">
            {sliderGorseller.map((slide, index) => (
              ...
              >
                ...
                />
                {/* Sadece Altta Yarı Saydam Başlık Şeridi */}
                <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent pt-12 pb-4 px-6">
                  <h2 className="text-white text-lg sm:text-xl font-bold drop-shadow-md">
                    {slide.baslik}
                  </h2>
                </div>
              </div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
            {modeller.map((item) => (
              ...
              >
                <div className="h-32 sm:h-48 bg-slate-100 relative overflow-hidden flex items-center justify-center text-slate-400 font-medium text-xs">
                  <img
                    src={item.resim}
                    alt={item.baslik}
          ...
          <div className="text-center mt-8">
            <Link
              href="/modeller"
              className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm py-2.5 px-6 rounded-lg shadow-sm transition-all"
            >
